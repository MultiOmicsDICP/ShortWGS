...
5： HaplotypeCaller  
6： GenotypeGVCFs（変異）  
7： SNP, INDELごとに、VariantFiltration  
8： Filtered VCF（FILTER列PASS抽出）    
...
gatk MarkDuplicates \
    -I SRR768162.bam \
    -O SRR768162_markdup.bam \
    -M SRR768162_markdup_metrics.txt \
...
```
samtools sort -O BAM -o SRR768162_markdup_sorted.bam SRR768162_markdup.bam

samtools index SRR768162_markdup_sorted.bam
```
...
gatk CollectInsertSizeMetrics \
  -I SRR768162_markdup_sorted.bam \
  -O SRR768162_markdup_sorted_insertsize_metrics.txt \
  -H SRR768162_markdup_sorted_insertsize_histogram.pdf
```

BAMファイルの詳細統計情報を出力するQC。
```bash
samtools stats SRR768162_markdup_sorted.bam > SRR768162_markdup_sorted_stats.txt
```

insert size、GC bias、coverageなどをグラフとして確認。
```bash
plot-bamstats -p bamstats SRR768162_markdup_sorted_stats.txt
```
...
gatk BaseRecalibrator \
    ...
    -I SRR768162_markdup_sorted.bam \
    -O SRR768162_markdup_recal.table

gatk ApplyBQSR \
    -R Homo_sapiens_assembly38.fasta \
    --bqsr-recal-file SRR768162_dedup_recal.table \
    -I SRR768162_markdup_sorted.bam \
    -O SRR768162_markdup_recal.bam
```
