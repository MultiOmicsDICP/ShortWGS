...
解析実行編  
マニュアル化のためのミーティング後、みんなのスクリプトを参考にまとめてみました。
...
bwa mem \
    ...
    samtools sort -@4 -o SRR768162_sorted.bam　#samを出さずにbamファイル出す
```
...
```bash
samtools sort -@4 -o SRR768162_sorted.bam SRR768162.bam
...
#### インデックス作成
```bash
samtools index SRR768162_dedup_resorted.bam
```
...
```bash
samtools flagstat SRR768162_dedup_resorted.bam > SRR768162_dedup_resorted_flagstat.txt
```

duplicate read除去後のlibrary insert size 分布を評価する。
```bash
gatk CollectInsertSizeMetrics \
  -I SRR768162_dedup_resorted.bam \
  -O SRR768162_dedup_sorted_insertsize_metrics.txt \
  -H SRR768162_dedup_sorted_insertsize_histogram.pdf
```

BAMファイルの詳細統計情報を出力するQC。
```bash
samtools stats SRR768162_dedup_resorted.bam > SRR768162_dedup_resorted_stats.txt
```

insert size、GC bias、coverageなどをグラフとして確認。
```bash
plot-bamstats -p bamstats SRR768162_dedup_resorted_stats.txt
```
...
gatk BaseRecalibrator \
    ...
    -I SRR768162_dedup_resorted.bam \
    -O SRR768162_dedup_resorted_recal.table

gatk ApplyBQSR \
    -R Homo_sapiens_assembly38.fasta \
    --bqsr-recal-file SRR768162_dedup_resorted_recal.table \
    -I SRR768162_dedup_resorted.bam \
    -O SRR768162_dedup_resorted_recal.bam
```
...
gatk HaplotypeCaller \
  -R Homo_sapiens_assembly38.fasta \
  -I SRR768162_dedup_resorted_recal.bam \
...
gatk IndexFeatureFile \
  -I SRR768162_filtered_snps_indels_PASS.vcf.gz
```
<br>
...
```bash
# stats出力
bcftools stats SRR768162_filtered_snps_indels_PASS.vcf.gz \
  > SRR768162_filtered_snps_indels_PASS_stats.txt
